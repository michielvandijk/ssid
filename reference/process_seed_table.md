...
Helper for `ipu()`. Strips columns from seed table except for the
primary id and marginal column (as reflected in the targets tables).
Also identifies factor columns with one level and processes them before
...
- df:
  ...
  the `data.frame` as processed by `ipu()` before this function is
  called.
